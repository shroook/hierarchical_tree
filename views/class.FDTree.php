    ...
    class FDTree
    {
        ...
        */
        public function __construct($tblName,$id,$title,$parent,$ajaxAddress,$imagesPath,$prefix = 'FDTree',$confirmMessage)
        {
            ...
            $q = "SELECT $id,$title,$parent FROM ".$tblName ." ORDER BY $id ASC";
        ...
        protected function createTree($array, $currentParent, $currLevel = 0, $prevLevel = -1) 
        {
            foreach ($array as $categoryId => $category) 
            {
                ...
                {                       
                    ...
                    $this->tree .= '<li>'.$label.'<form enctype="multipart/form-data" class="form-horizontal" method="post" action="../insert.php" name="basic_validate" id="basic_validate" novalidate="novalidate" >
                    <input type="file" value="'.$categoryId.'""  id="'.$categoryId.'""  name="image" value="" accept=".png,.gif,.jpg,.webp" require >
                    <span>Add Image</span><input type="submit" class="noText" value="'.$categoryId.'"" id="'.$categoryId.'""  name="add"  require></form><input type="checkbox" id="'.$this->prefix.'sub'.$categoryId.'">';
