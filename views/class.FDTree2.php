<?php
include_once("products_function.php");
        ...
        protected function createTree($array, $currentParent, $currLevel = 0, $prevLevel = -1) 
        {
            ...
            foreach ($array as $categoryId => $category) 
            {
                ...
                {                      
                    ...
                    //return chks 
                    $checked_arr=array();
                    $fetchall=mysqli_query($conn,"SELECT * FROM product_category WHERE product_id='" . $_GET['product_id'] . "'");
                    $result = mysqli_fetch_assoc($fetchall);
                    if(mysqli_num_rows($fetchall) > 0){
                    
                      $checked_arr = explode(',',$result['category_id']);
                    ...
                    if(count($id) > 0)
                        ...
                        $label = '<span id="'.$this->prefix.'lastNod'.$categoryId.'"
                         onclick="'.$this->prefix.'nod('.$categoryId.')">'.$category[$this->titleField].'</span>';
                    $this->tree .= '<li>'.$label.' 
                    
                   <input type="checkbox" name="parent_id[]" value="'.$categoryId.'" id="'.'sub'.$categoryId.'" > <label <label class="labelTree" for="'.$this->prefix.'sub'.$categoryId.'">'.$category[$this->titleField].'</label><i data-id'.$this->prefix.'="'.$categoryId.'" class="fa fa-pencil"></i><i onclick="'.$this->prefix.'addSubCat('.$categoryId.')" class="fa fa-plus" ></i>';
